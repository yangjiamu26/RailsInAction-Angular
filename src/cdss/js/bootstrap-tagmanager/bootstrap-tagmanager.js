...
(function (jQuery) {
  ...
  jQuery.fn.tagsManager = function (options,tagToManipulate) {
    ...
    var pushTag = function (tag, objToPush, isValid) {
      ...
      // var tlisLowerCase = tlis.map(function(elem) { return elem.toLowerCase(); });//不兼容IE 
      var tlisLowerCase = [];
      for(var i = 0; i < tlis.length; i++)
      {
          tlisLowerCase[i] = tlis[i].toLowerCase();
      }
    ...
    return this.each(function () {
      ...
      if (tagManagerOptions.hiddenTagListId == null) { /* if hidden input not given default activity */
        ...
        var html = "";
        html += "<input name='" + tagManagerOptions.hiddenTagListName + "' id='"+tagManagerOptions.hiddenTagListName+"' type='hidden' value=''/>";
        obj.after(html);
