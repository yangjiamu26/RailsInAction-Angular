...
.controller('monitorIndexCtrl', function($scope, $state, Datacenters){
  if($state.params.dcId){
    $state.go('app.resourcemonitor.monitor.show',{dcId:$state.params.dcId});
  }else{
    Datacenters.query(function(res){
      $state.go('app.resourcemonitor.monitor.show',{dcId:res.data[0].id});
    })
  }
})
.controller('monitorShowCtrl', function($scope, $rootScope, $timeout, $state, Datacenters, MonitorDatacentersStatics, MonitorDatacentersHosts, MonitorDcAlarmsStatics) {
    ...
    $scope.getResouces = function(dcId){
      ...
      MonitorDcAlarmsStatics.query({type:'alarmsDetail',dcId:dcId},function(val){
        $rootScope.root_warnings = val.data;
      })
      MonitorDcAlarmsStatics.query({type:'alarmsDetail',dcId:dcId,hostType:'host'},function(val){
        ...
        function DangerInterval(){
          $scope.myTimeout1 = $timeout(function(){
        ...
        DangerInterval();
        $scope.alarmsHover = function(){
          $timeout.cancel($scope.myTimeout1);
          $timeout.cancel($scope.myTimeout2);
        }
        $scope.alarmsLeave = function(){
          DangerInterval();
        }
        $scope.closeArlams = function(){
          $scope.showDangerbox = false;
          $timeout.cancel($scope.myTimeout1);
          $timeout.cancel($scope.myTimeout2);
          $scope.hideArlams = true;
        }
        $scope.showAlarms = function(){
          DangerInterval();
          $scope.hideArlams = false;
        }
      });
    }
    ...
    $scope.changeDc = function(){
      $state.go('app.resourcemonitor.monitor.show',{dcId:$scope.selected_dcId});
    }
    ...
    debugger
      ...
      if(obj.freeHosts.length>0){
        for(var i=0;i<obj.freeHosts.length;i++){
          ...
          var edge3 = {"target": arr.id,"source": "1001"};
          nodes.push(arr);
          edges.push(edge3);
        }
      }
      ...
      $scope.cpotoOptions = {
          ...
          style:{                    //说明: 全局样式
            global:{
              backgroundColor:"#fafafa",  //说明: 支持fillstyle所有原生写法, 例: rgba(255,255,255,1),默认#ffffff
