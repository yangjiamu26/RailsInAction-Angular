...
var KisBpmStringPropertyCtrl = [ '$scope', function ($scope) {
    ...
    /*监听输入值*/
    $scope.$watch('property.value', function (newVal,oldVal,$scope) {
        if (!newVal && $scope.property.required) {
            $scope.$emit("stopFunctioned" , true);
        }
        if(newVal && $scope.property.required){
        	$scope.$emit("stopFunctioned" , false);
        }
    });
    /*监听输入值*/
    ...
    $scope.inputBlurred = function(e) {
    	if(e.target.value == '' && $scope.property.required){
    		e.target.focus();
    	}
    ...
    $scope.enterPressed = function(keyEvent) {
    	if (keyEvent && keyEvent.which === 13) {
	        ...
	        $scope.inputBlurred(keyEvent); // we want to do the same as if the user would blur the input field
    	}
    };
