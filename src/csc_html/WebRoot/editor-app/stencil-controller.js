...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
    	...
    	$scope.stopFunctioned = false;
    	$scope.$on('stopFunctioned',function(e,data){ 
    		$scope.stopFunctioned = data;
    	});
          ...
          //modified by zhouheng 2016-1-6 start
            ...
            $http({method: 'GET', url: modelIdUrl}).
               success(function (data, status, headers, config) {            	   
            	   angular.forEach(data.pageNodeList,function(v,k){
            		   this.push(v);
            	   },formKeyList);
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            //added by zhouheng start
                            if (key === 'oryx-formkeydefinition') {                           	
                            	propertyConfig = KISBPM.PROPERTY_CONFIG["kisbpm-formkey"];;
                            }
                          //added by zhouheng end
                        }
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            var currentProperty = {
                                ...
                                'required': property._jsonProp.required ? property._jsonProp.required : false,
                                'readonly': property._jsonProp.readonly ? property._jsonProp.readonly : false
                            };
                            
                            //add by zhouheng 2016-1-7 start
                            if (key === 'oryx-formkeydefinition') {
                            	//debugger
                            	currentProperty.formKeys = formKeyList;
                            }
                            
                            if(key === 'oryx-usertaskassignment'){
                            	//debugger
                            }
                            //add by zhouheng 2016-1-7 end
        ...
        $scope.propertyClicked = function (index) {
        	if($scope.selectedItem.properties[index].readonly) return false;
        ...
        $scope.updatePropertyInModel = function (property, shapeId, el) {
        	if($scope.stopFunctioned) return;
