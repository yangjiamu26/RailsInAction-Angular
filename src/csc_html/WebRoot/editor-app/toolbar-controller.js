...
angular.module('activitiModeler')
    .controller('ToolbarController', ['$scope', '$http', '$modal', '$q', '$rootScope', '$translate', '$location', function ($scope, $http, $modal, $q, $rootScope, $translate, $location) {
        ...
        $scope.toolbarButtonClicked = function(buttonIndex) {
        	//debugger
        	//modified by zhouheng 2016-1-6 增加非空校验 start
        	if(buttonIndex == 0){        		        	
	        	var properties = $scope.selectedItem.properties;
	        	var data = $scope.editor.getJSON();
	        	if(data.properties.name == ""){
	        		var msg = "流程定义名称字段不能为空！\r\n";
	        		alert(msg);
	        		return;
	        	}
	        	var data1 = data.childShapes;
	        	for(var i = 0;i<data1.length;i++){
	        		if(typeof(data1[i].properties.asynchronousdefinition) != "undefined"){
	        			if(data1[i].properties.name == ""){
	    	        		var msg = "流程定义节点名称字段不能为空！\r\n";
	    	        		alert(msg);
	    	        		return;
	    	        	}
	        		}
        	...
        	//modified by zhouheng 2016-1-6 增加非空校验 end
            // Default behaviour
