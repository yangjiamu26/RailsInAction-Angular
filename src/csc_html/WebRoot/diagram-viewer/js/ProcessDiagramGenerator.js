...
var ProcessDiagramGenerator = {	
	...
	drawHighLights: function(processInstanceId) {
		...
		*/		
		...
		ActivitiRest.getProcessTaskList(processInstanceId, this._drawProcessTask);
	},
	//获取流程审批任务详情
	_drawProcessTask:function(){
		var processTasks = this.processTasks;
		var tables = "<table style='text-align:left;border:1px solid #ddd;border-spacing:0;margin-top:10px;border-collapse:collapse;width:900px;display:table-row;'>" +
				"<thead><tr style='display:table-row;vertical-align:inherit;border-color:inherit;border-bottom:1px solid #dddddd !important;'>" +
				"<th width='180px' style='border-left: none;'>节点名称</th>" +
				"<th width='120px' style='border-left: none;'>办理人</th>" +
				"<th width='200px' style='border-left: none;'>办理时间</th>" +
				"<th width='130px' style='border-left: none;'>办理结果</th>" +
				"<th width='200px' style='border-left: none;'>办理意见</th>" +
				"</tr></thead><tbody >";
		for(var i = 0;i<processTasks.length;i++){
			var comment = processTasks[i].comment;
			if(comment.length > 19){
				comment = comment.substring(0,17)+"...";
			}		
			tables += "<tr style='border-bottom:1px solid #dddddd;display:table-row;'>" +
					"<td style='border-top:1px solid #dddddd;border-left: none;'>"+processTasks[i].name+
					"</td><td style='border-top:1px solid #dddddd;border-left: none;'>"+processTasks[i].assiginee+
				   "</td><td style='border-top:1px solid #dddddd;border-left: none;'>"+processTasks[i].time+
					"</td><td style='border-top:1px solid #dddddd;border-left: none;'>"+processTasks[i].result+
					"</td><td style='border-top:1px solid #dddddd;word-wrap:break-word;word-break:break-all;width:200px;border-left: medium none;' title='"+processTasks[i].comment+"'>"+comment+"</td></tr>";
		}
		tables += "</tbody></table>";
		$("#tableInfo").html(tables);
...
//		diagramInfo.html(tpl); modified by zhouheng 2016-1-20暂时屏蔽
	},
