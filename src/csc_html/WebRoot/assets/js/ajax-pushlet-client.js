...
// Get object reference
	...
	_getWebRoot: function() {
		...
		for (var i = 0; i < nodes.length; ++i) {
			...
			if (src) {
				var index = src.indexOf("commJs/ajax-pushlet-client.js");
				if (index >= 0) {
...
// On response  optional callback fun is called with optional user data.
	_getXML: function(url, callback) {
		...
		// Open URL
		var now=new Date();
		url=url+"&_timeflag_="+now.getTime()+Math.random();
		PL.debug("-my",url);
	...
	_onEvent: function (event) {
		...
		if (eventType == 'data') {
			PL._setStatus('data');
			PL._doCallback(event, window.onPushLetData);
			...
			PL._doCallback(event, window.onPlushletError);
