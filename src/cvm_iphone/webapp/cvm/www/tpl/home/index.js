myApp.onPageInit("home-index", function(page) {
  ...
  function ViewModel(){
    ...
    this.loadData = function(DcId){
      ...
      RestServiceJs.query(url1,{},function(data){
        ...
        data.memoryTotal = Number((Number(data.memorySize||data.memoryTotal)/1024).toFixed(2));
        data.storageTotal = Number((Number(data.storageSize||data.storageTotal)/1024).toFixed(2));
        self.infos(data);
      });
  ...
  $$(page.container).find('.pull-to-refresh-content').on('refresh', function (e) {
    viewModel.loadData(viewModel.whichDc());
  });
